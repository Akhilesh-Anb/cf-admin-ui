...
ServiceBindingsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    this.addFilterRowIfValue(table, "Service Binding Events", Format.formatNumber, row[7], serviceBinding.guid, AdminUI.showEvents);
    ...
    // We can have a case where the service broker returns an empty array for volume_mounts.
    // Handle as empty subtable thereby showing the actual data returned by the service broker.
    if ((volumeMounts != null) && (row[6] === true))
    {
