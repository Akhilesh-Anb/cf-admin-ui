...
ServicesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[14] != null)
    {
        for (var index = 0; index < row[14].length; index++)
        {
            var field = row[14][index];
    ...
    this.addFilterRowIfValue(table, "Service Events", Format.formatNumber, row[15], service.guid, AdminUI.showEvents);
    this.addFilterRowIfValue(table, "Service Plans", Format.formatNumber, row[16], service.guid, AdminUI.showServicePlans);
    this.addRowIfValue(this.addPropertyRow, table, "Public Active Service Plans", Format.formatNumber, row[17]);
    this.addFilterRowIfValue(table, "Service Plan Visibilities", Format.formatNumber, row[18], service.guid, AdminUI.showServicePlanVisibilities);
    this.addFilterRowIfValue(table, "Service Instances", Format.formatNumber, row[19], service.guid, AdminUI.showServiceInstances);
    this.addFilterRowIfValue(table, "Service Instance Shares", Format.formatNumber, row[20], service.guid, AdminUI.showSharedServiceInstances);
    this.addFilterRowIfValue(table, "Service Bindings", Format.formatNumber, row[21], service.guid, AdminUI.showServiceBindings);
    this.addFilterRowIfValue(table, "Service Keys", Format.formatNumber, row[22], service.guid, AdminUI.showServiceKeys);
    this.addFilterRowIfValue(table, "Route Bindings", Format.formatNumber, row[23], service.guid, AdminUI.showRouteBindings);
