...
ServicesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[11] != null)
    {
        for (var index = 0; index < row[11].length; index++)
        {
            var field = row[11][index];
    ...
    this.addFilterRowIfValue(table, "Service Events", Format.formatNumber, row[12], service.guid, AdminUI.showEvents);
    this.addFilterRowIfValue(table, "Service Plans", Format.formatNumber, row[13], service.guid, AdminUI.showServicePlans);
    this.addRowIfValue(this.addPropertyRow, table, "Public Active Service Plans", Format.formatNumber, row[14]);
    this.addFilterRowIfValue(table, "Service Plan Visibilities", Format.formatNumber, row[15], service.guid, AdminUI.showServicePlanVisibilities);
    this.addFilterRowIfValue(table, "Service Instances", Format.formatNumber, row[16], service.guid, AdminUI.showServiceInstances);
    this.addFilterRowIfValue(table, "Service Bindings", Format.formatNumber, row[17], service.guid, AdminUI.showServiceBindings);
    this.addFilterRowIfValue(table, "Service Keys", Format.formatNumber, row[18], service.guid, AdminUI.showServiceKeys);
    this.addFilterRowIfValue(table, "Route Bindings", Format.formatNumber, row[19], service.guid, AdminUI.showRouteBindings);
